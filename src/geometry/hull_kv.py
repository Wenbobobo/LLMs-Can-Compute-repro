...
class HullKVCache:
    ...
    def query(self, query: Sequence[NumberLike]) -> HardmaxResult:
        ...
        # Optimize dot product calculation for already coerced fraction keys to avoid _coerce_key overhead
        kx, ky = point.key
        score = (kx * qx) + (ky * qy)
    ...
    def _scan_maximizers(
        ...
        for point in self._points:
            # Inline exact fraction dot product to avoid repeated tuple coercion and type-checking in dot_2d
            kx, ky = point.key
            score = (kx * qx) + (ky * qy)
