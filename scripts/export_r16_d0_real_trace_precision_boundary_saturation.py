"""Export the bounded R16 real-trace precision saturation on the admitted R8/R15 surface."""
...
OUT_DIR = ROOT / "results" / "R16_d0_real_trace_precision_boundary_saturation"
...
def load_admitted_surface_cases() -> tuple[list[dict[str, object]], dict[str, object]]:
    case_specs = (
        (
    ...
    surface_cases: list[dict[str, object]] = []
    ...
    for source_lane, exact_suite_path, cases in case_specs:
        ...
        lane_cases = [
            {"source_lane": source_lane, "case": case}
            for case in cases
        ...
        source_rows.append(
            {
                ...
                "exact_suite_row_count": len(exact_payload["rows"]),
                "admitted_program_count": len(lane_cases),
                "families": sorted({entry["case"].family for entry in lane_cases}),
                "program_names": sorted(entry["case"].program.name for entry in lane_cases),
            }
        )
        surface_cases.extend(lane_cases)
...
def build_screening(surface_cases: list[dict[str, object]]) -> tuple[dict[str, object], list[dict[str, object]]]:
    interpreter = TraceInterpreter()
    streams: dict[str, dict[str, object]] = {}
    base_sweep_rows: list[dict[str, object]] = []

    for entry in sorted(surface_cases, key=lambda item: (item["source_lane"], item["case"].family, item["case"].program.name)):
        source_lane = str(entry["source_lane"])
        case = entry["case"]
    ...
    screening_payload = {
        "experiment": "r16_d0_real_trace_precision_screening",
        "environment": detect_runtime_environment().as_dict(),
        "notes": [
            "R16 stays same-scope and merges the exact-admitted R8 and R15 surfaces instead of opening a wider precision sweep.",
            "The bounded grid stays on memory streams only so the lane saturates the admitted real-trace precision surface without widening to arbitrary streams or stack-wide guarantees.",
            "Default base 64 defines the screened grid before any boundary-bearing stream receives a small decomposition base sweep.",
...
def build_family_boundary_rows(stream_rows: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    for family, family_stream_rows in sorted(grouped.items()):
        family_rows.append(
            {
                "family": family,
                "source_lanes": sorted({str(row["source_lane"]) for row in family_stream_rows}),
...
def build_failure_taxonomy_rows(
    ...
    for row in base_sweep_rows:
        if row["first_failure"] is not None:
            counter[str(row["first_failure"]["failure_type"])] += 1
    return [{"failure_type": failure_type, "count": count} for failure_type, count in sorted(counter.items())]


def build_source_surface_rows(surface_metadata: dict[str, object], stream_rows: list[dict[str, object]]) -> list[dict[str, object]]:
    stream_rows_by_lane: defaultdict[str, list[dict[str, object]]] = defaultdict(list)
    for row in stream_rows:
        stream_rows_by_lane[str(row["source_lane"])].append(row)

    rows: list[dict[str, object]] = []
    for source_row in surface_metadata["source_rows"]:
        source_lane = str(source_row["source_lane"])
        lane_stream_rows = stream_rows_by_lane[source_lane]
        rows.append(
            {
                "source_lane": source_lane,
                "exact_suite_row_count": int(source_row["exact_suite_row_count"]),
                "admitted_program_count": int(source_row["admitted_program_count"]),
                "screened_stream_count": len(lane_stream_rows),
                "family_count": len(source_row["families"]),
                "families": list(source_row["families"]),
                "boundary_bearing_stream_count": sum(bool(row["entered_boundary_followup"]) for row in lane_stream_rows),
                "effective_here_stream_count": sum(str(row["classification"]) == "effective_here" for row in lane_stream_rows),
                "unproven_here_stream_count": sum(str(row["classification"]) == "unproven_here" for row in lane_stream_rows),
                "negated_here_stream_count": sum(str(row["classification"]) == "negated_here" for row in lane_stream_rows),
            }
        )
    return rows


def build_runtime_bridge_handoff(stream_rows: list[dict[str, object]], family_rows: list[dict[str, object]]) -> dict[str, object]:
    boundary_bearing_streams = [row["stream_name"] for row in stream_rows if bool(row["entered_boundary_followup"])]
    return {
        "surface_state": "full_admitted_same_scope_precision_surface_screened",
        "screened_stream_count": len(stream_rows),
        "family_count": len(family_rows),
        "all_streams_effective_here": all(str(row["classification"]) == "effective_here" for row in stream_rows),
        "boundary_bearing_stream_count": len(boundary_bearing_streams),
        "boundary_bearing_streams": boundary_bearing_streams,
        "representative_precision_streams": [row["stream_name"] for row in stream_rows],
    ...
    *,
    ...
    return {
        "source_surface": {
            ...
            "admitted_program_count": int(surface_metadata["admitted_program_count"]),
        ...
        "claim_impact": {
            ...
            "distilled_boundary": {
                "effective_here": [
                    f"The admitted same-scope R8/R15 memory surface remains precision-stable on {effective_count}/{len(stream_rows)} screened streams under the bounded grid."
                ],
                "unproven_here": [
                    f"{unproven_count}/{len(stream_rows)} screened streams still need decomposition recovery or remain boundary-bearing, so broader robustness remains unproven."
                ],
                "negated_here": [
                    f"Single-head float32 fails without screened recovery on {negated_count}/{len(stream_rows)} screened streams, and the weaker base-256 control fails on {len(negative_control_failures)}/{len(negative_control_rows)} boundary-bearing follow-ups."
                ],
            },
            "supported_here": [
                "R16 saturates the admitted same-scope real-trace precision surface by merging the exact-admitted R8 and R15 families without widening endpoint scope.",
                "The lane keeps failure taxonomy, decomposition recovery, and weaker negative controls explicit at stream and family granularity before runtime follow-up work resumes.",
            ],
            "unsupported_here": [
                "R16 does not justify unseen-family precision guarantees, a broader stack-wide robustness claim, or arbitrary compiled-language support.",
                "R16 does not by itself establish same-endpoint runtime superiority; that remains downstream of R17.",
            ],
        },
    }
...
def main() -> None:
    ...
    screening_payload, base_sweep_rows = build_screening(surface_cases)
    ...
    source_surface_rows = build_source_surface_rows(surface_metadata, stream_rows)
    runtime_bridge_handoff = build_runtime_bridge_handoff(stream_rows, family_rows)
    summary = build_summary(
        surface_metadata=surface_metadata,
        stream_rows=stream_rows,
        family_rows=family_rows,
        source_surface_rows=source_surface_rows,
    ...
    write_json(
        ...
        {
            "experiment": "r16_d0_real_trace_precision_boundary_saturation",
    ...
    write_json(
        ...
        {
            **screening_payload,
            "rows_from_exact_suites": int(surface_metadata["exact_suite_row_count"]),
            "admitted_program_count": int(surface_metadata["admitted_program_count"]),
        },
    )
    write_json(OUT_DIR / "stream_boundary_summary.json", {"experiment": "r16_stream_boundary_summary", "environment": environment.as_dict(), "rows": stream_rows})
    write_json(OUT_DIR / "family_boundary_summary.json", {"experiment": "r16_family_boundary_summary", "environment": environment.as_dict(), "rows": family_rows})
    write_json(OUT_DIR / "source_surface_summary.json", {"experiment": "r16_source_surface_summary", "environment": environment.as_dict(), "rows": source_surface_rows})
    write_json(OUT_DIR / "negative_control_rows.json", {"experiment": "r16_negative_control_rows", "environment": environment.as_dict(), "rows": [row for row in base_sweep_rows if row["negative_control_kind"] is not None]})
    write_json(OUT_DIR / "failure_taxonomy.json", {"experiment": "r16_failure_taxonomy", "environment": environment.as_dict(), "rows": failure_taxonomy_rows})
    write_json(OUT_DIR / "claim_impact.json", {"experiment": "r16_claim_impact", "environment": environment.as_dict(), "summary": summary["claim_impact"]})
    write_json(OUT_DIR / "runtime_bridge_handoff.json", {"experiment": "r16_runtime_bridge_handoff", "environment": environment.as_dict(), "summary": runtime_bridge_handoff})
    (OUT_DIR / "README.md").write_text(
        "\n".join(
            [
                "# R16 D0 Real-Trace Precision Boundary Saturation",
                "",
                "Artifacts:",
                "- `summary.json`",
                "- `screening.json`",
                ...
                "- `source_surface_summary.json`",
                "- `negative_control_rows.json`",
                "- `failure_taxonomy.json`",
                "- `claim_impact.json`",
                "- `runtime_bridge_handoff.json`",
            ]
        )
        + "\n",
        encoding="utf-8",
