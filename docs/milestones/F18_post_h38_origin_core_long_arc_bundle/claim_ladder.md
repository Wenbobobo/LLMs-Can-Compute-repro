...
| `D` | a restricted Wasm / tiny-`C` lowering can run useful kernels exactly on the same append-only substrate | `not_yet_validated_here` | `R44_origin_restricted_wasm_useful_case_execution_gate` | any useful kernel needs heap, alias-heavy pointers, hidden mutable state, or approximation | keep as bounded future surface only |
