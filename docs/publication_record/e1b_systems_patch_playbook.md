...
current frozen scope. The target is the mixed `R2` systems gate: geometry
asymptotics are positive, but the lowered current-scope execution path is not
yet end-to-end competitive on the current positive `D0` suites.
...
- a review or release requirement demands one stronger systems artifact on the
  present `D0` scope and cannot be satisfied by wording cleanup alone;
...
`E1b` is allowed to reason only about the current mixed systems gate:

- primary scope: `R2` and its role in blocking automatic frontend widening;
- dependent read-only context: the current `D0` suites and the `M7`
...
1. one rerun of the current same-scope runtime profile bundle on the already
   positive `D0` suites;
...
The lane must keep the comparison surface identical to the current scope unless
an explicit new scope decision is recorded elsewhere.

## Execution protocol

1. Quote the triggering systems conflict and the exact sentence/table/review
   request that depends on it.
2. State the gate question in yes/no form on current scope, for example whether
   the lowered path is competitive enough to justify a narrower wording change.
3. Freeze the comparison surface to the current positive `D0` suites and the
   existing geometry/interpreter/lowered/spec paths.
